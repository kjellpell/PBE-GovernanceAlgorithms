...
This is the same quantity as `Fristprosent (måned)` (a live DAX measure against `Faser`,
defined elsewhere in the model), written differently — they give the same headline numbers, but do not assume from that
